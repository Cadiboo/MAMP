...
class PHPWebSocket
{
	...
	const SOCKET = 0;									                 		// resource	- client socket
	const BUFFER = 1;									                 		// string		- a blank string when there's no incoming frames
	const READY_STATE = 2;						                 		// integer	- between 0 and 3
	const LAST_RECV_TIME = 3;					                 		// integer	- set to time() when the client is added
	const PING_SENT_TIME = 4;						                 	// integer/false	- false when the server is not waiting for a pong
	const CLOSE_STATUS = 5;							                 	// integer/false	- close status that wsOnClose() will be called with
	const IPv4 = 6;											                 	// integer	- client's IP stored as a signed long, retrieved from ip2long()
	const FRAME_PAYLOAD_DATA_LENGTH = 7;                 	// integer/false	- length of a frame's payload data, reset to false when all frame data has been read (cannot reset to 0, to allow reading of mask key)
	const FRAME_BYTES_READ = 8;					                 	// integer	- amount of bytes read for a frame, reset to 0 when all frame data has been read
	const FRAME_BUFFER = 9;							                 	// string		- joined onto end as a frame's data comes in, reset to blank string when all frame data has been read
	const MESSAGE_OPCODE = 10;					                 	// integer	-  stored by the first frame for fragmented messages, default value is 0
	const MESSAGE_BUFFER_LENGTH = 11;		                 	// integer	-  the payload data length of MessageBuffer
	const HEADERS = 12;									                 	// array		-  the heders recieved before the connection was upgraded
	const SESSION = 13;									                 	// array		-  $_SESSION
	/*
	...
	*/
	...
	function wsStartServer($host, $port) {
		...
		if (!socket_bind($this->wsRead[0], $host, $port)) {
			socket_close($this->wsRead[0]);
			$this->log("Could not bind $host, $port");
			return false;
		}
	...
	function wsStopServer() {
		...
		foreach ($this->wsClients as $clientID => $client) {
			...
			if ($client[self::READY_STATE] != self::WS_READY_STATE_CONNECTING) {
				$this->wsSendClientClose($clientID, self::WS_STATUS_GONE_AWAY);
			}
			socket_close($client[self::SOCKET]);
		}
	...
	function wsCheckIdleClients() {
		$time = time();
		foreach ($this->wsClients as $clientID => $client) {
			if ($client[self::READY_STATE] != self::WS_READY_STATE_CLOSED) {
				// client ready state is not closed
				if ($client[self::PING_SENT_TIME] !== false) {
					// ping request has already been sent to client, pending a pong reply
					if ($time >= $client[self::PING_SENT_TIME] + self::WS_TIMEOUT_PONG) {
				...
				elseif ($time >= $client[self::LAST_RECV_TIME] + self::WS_TIMEOUT_RECV) {
					// last data was received >= self::WS_TIMEOUT_RECV seconds ago
					if ($client[self::READY_STATE] != self::WS_READY_STATE_CONNECTING) {
						// client ready state is open or closing
						$this->wsClients[$clientID][self::PING_SENT_TIME] = time();
	...
	function wsGetClientSocket($clientID) {
		return $this->wsClients[$clientID][self::SOCKET];
	}

	// client read functions
	function wsProcessClient($clientID, &$buffer, $bufferLength) {
		if ($this->wsClients[$clientID][self::READY_STATE] == self::WS_READY_STATE_OPEN) {
			// handshake completed
			$result = $this->wsBuildClientFrame($clientID, $buffer, $bufferLength);
		}
		elseif ($this->wsClients[$clientID][self::READY_STATE] == self::WS_READY_STATE_CONNECTING) {
			// handshake not completed
			$result = $this->wsProcessClientHandshake($clientID, $buffer);
			if ($result) {
				$this->wsClients[$clientID][self::READY_STATE] = self::WS_READY_STATE_OPEN;
				$this->wsClients[$clientID][self::SESSION] = $_SESSION;
	...
	function wsBuildClientFrame($clientID, &$buffer, $bufferLength) {
		...
		$this->wsClients[$clientID][self::FRAME_BYTES_READ] += $bufferLength;
		$this->wsClients[$clientID][self::FRAME_BUFFER] .= $buffer;

		// check if the length of the frame's payload data has been fetched, if not then attempt to fetch it from the frame buffer
		if ($this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] !== false || $this->wsCheckSizeClientFrame($clientID) == true) {
			// work out the header length of the frame
			$headerLength = ($this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] <= 125 ? 0 : ($this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] <= 65535 ? 2 : 8)) + 6;

			// check if all bytes have been received for the frame
			$frameLength = $this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] + $headerLength;
			if ($this->wsClients[$clientID][self::FRAME_BYTES_READ] >= $frameLength) {
				// check if too many bytes have been read for the frame (they are part of the next frame)
				$nextFrameBytesLength = $this->wsClients[$clientID][self::FRAME_BYTES_READ] - $frameLength;
				if ($nextFrameBytesLength > 0) {
					$this->wsClients[$clientID][self::FRAME_BYTES_READ] -= $nextFrameBytesLength;
					$nextFrameBytes = substr($this->wsClients[$clientID][self::FRAME_BUFFER], $frameLength);
					$this->wsClients[$clientID][self::FRAME_BUFFER] = substr($this->wsClients[$clientID][self::FRAME_BUFFER], 0, $frameLength);
				}
				...
				if (isset($this->wsClients[$clientID])) {
					$this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] = false;
					$this->wsClients[$clientID][self::FRAME_BYTES_READ] = 0;
					$this->wsClients[$clientID][self::FRAME_BUFFER] = '';
				}
	...
	function wsCheckSizeClientFrame($clientID) {
		...
		if ($this->wsClients[$clientID][self::FRAME_BYTES_READ] > 1) {
			// fetch payload length in byte 2, max will be 127
			$payloadLength = ord(substr($this->wsClients[$clientID][self::FRAME_BUFFER], 1, 1)) & 127;

			if ($payloadLength <= 125) {
				// actual payload length is <= 125
				$this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] = $payloadLength;
			}
			elseif ($payloadLength == 126) {
				// actual payload length is <= 65,535
				if (substr($this->wsClients[$clientID][self::FRAME_BUFFER], 3, 1) !== false) {
					// at least another 2 bytes are set
					$payloadLengthExtended = substr($this->wsClients[$clientID][self::FRAME_BUFFER], 2, 2);
					$array = unpack('na', $payloadLengthExtended);
					$this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] = $array['a'];
				}
			}
			else {
				// actual payload length is > 65,535
				if (substr($this->wsClients[$clientID][self::FRAME_BUFFER], 9, 1) !== false) {
					// at least another 8 bytes are set
					$payloadLengthExtended = substr($this->wsClients[$clientID][self::FRAME_BUFFER], 2, 8);
			...
			if ($this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] !== false) {

				// check if the frame's payload data length exceeds self::WS_MAX_FRAME_PAYLOAD_RECV
				if ($this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] > self::WS_MAX_FRAME_PAYLOAD_RECV) {
					$this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] = false;
					$this->wsSendClientClose($clientID, self::WS_STATUS_MESSAGE_TOO_BIG);
					return false;
				}
				...
				$controlFrame = (ord(substr($this->wsClients[$clientID][self::FRAME_BUFFER], 0, 1)) & 8) == 8;
				if (!$controlFrame) {
					$newMessagePayloadLength = $this->wsClients[$clientID][self::MESSAGE_BUFFER_LENGTH] + $this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH];
	...
	function wsProcessClientFrame($clientID) {
		...
		$this->wsClients[$clientID][self::LAST_RECV_TIME] = time();

		// fetch frame buffer
		$buffer = &$this->wsClients[$clientID][self::FRAME_BUFFER];
		...
		$seek = $this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] <= 125 ? 2 : ($this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH] <= 65535 ? 4 : 10);
		...
		if ($opcode != self::WS_OPCODE_CONTINUATION && $this->wsClients[$clientID][self::MESSAGE_BUFFER_LENGTH] > 0) {
			// clear the message buffer
			$this->wsClients[$clientID][self::MESSAGE_BUFFER_LENGTH] = 0;
			$this->wsClients[$clientID][self::BUFFER] = '';
		}
		...
		if ($fin == self::WS_FIN) {
			...
			if ($opcode != self::WS_OPCODE_CONTINUATION) {
				// process the message
				return $this->wsProcessClientMessage($clientID, $opcode, $data, $this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH]);
			}
			else {
				// increase message payload data length
				$this->wsClients[$clientID][self::MESSAGE_BUFFER_LENGTH] += $this->wsClients[$clientID][self::FRAME_PAYLOAD_DATA_LENGTH];

				// push frame payload data onto message buffer
				$this->wsClients[$clientID][self::BUFFER] .= $data;

				// process the message
				$result = $this->wsProcessClientMessage($clientID, $this->wsClients[$clientID][self::MESSAGE_OPCODE], $this->wsClients[$clientID][self::BUFFER], $this->wsClients[$clientID][self::MESSAGE_BUFFER_LENGTH]);

				// check if the client wasn't removed, then reset message buffer and message opcode
				if (isset($this->wsClients[$clientID])) {
					$this->wsClients[$clientID][self::BUFFER] = '';
					$this->wsClients[$clientID][self::MESSAGE_OPCODE] = 0;
					$this->wsClients[$clientID][self::MESSAGE_BUFFER_LENGTH] = 0;
				}
			...
			if ($opcode != self::WS_OPCODE_CONTINUATION) {
				$this->wsClients[$clientID][self::MESSAGE_OPCODE] = $opcode;
			}
		}
	...
	function wsProcessClientMessage($clientID, $opcode, &$data, $dataLength) {
		...
		elseif ($opcode == self::WS_OPCODE_PONG) {
			...
			if ($this->wsClients[$clientID][self::PING_SENT_TIME] !== false) {
				$this->wsClients[$clientID][self::PING_SENT_TIME] = false;
			}
		}
		elseif ($opcode == self::WS_OPCODE_CLOSE) {
			...
			if ($this->wsClients[$clientID][self::READY_STATE] == self::WS_READY_STATE_CLOSING) {
				...
				$this->wsClients[$clientID][self::READY_STATE] = self::WS_READY_STATE_CLOSED;
			}
			else {
	...
	function wsProcessClientHandshake($clientID, &$buffer) {
		...
		//set client headers
		$this->wsClients[$clientID][self::HEADERS] = $headersKeyed;

		//set client session
		$this->wsClients[$clientID][self::SESSION] = $this->loadSession($clientID);
	...
	function wsSendClientMessage($clientID, $opcode, $message) {
		...
		if ($this->wsClients[$clientID][self::READY_STATE] == self::WS_READY_STATE_CLOSING || $this->wsClients[$clientID][self::READY_STATE] == self::WS_READY_STATE_CLOSED) return true;
	...
	function wsSendClientClose($clientID, $status=false) {
		...
		// store close status
		$this->wsClients[$clientID][self::CLOSE_STATUS] = $status;
		...
		$this->wsClients[$clientID][self::READY_STATE] = self::WS_READY_STATE_CLOSING;
	}
	...
	function wsSend($clientID, $message, $binary=false) {
		if ( array_key_exists('send', $this->wsOnEvents) )
			foreach ( $this->wsOnEvents['send'] as $func )
				$func($clientID, $message, $binary);
	...
	function loadSession ($clientID) {
		...
		$sessionid = $this->getSessionID($clientID);
	...
	function getSessionID ($clientID) {
		$headersKeyed = $this->wsClients[$clientID][self::HEADERS];
		return explode("; ", explode("PHPSESSID=", $headersKeyed['Cookie'])[1])[0];
	}

	function log( $message )
	{
		echo date('[Y-m-d H:i:s] ') . $message . "\033[0m\n";
	}
